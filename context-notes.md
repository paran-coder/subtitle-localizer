...
## 2026-09-10 번역 언어 확장
- 번역 대상에 영어 `en`과 러시아어 `ru`를 추가합니다.
- 한국어를 포함한 어떤 원본 언어도 영어 원문으로 가정하지 않고 입력 자막 자체를 해석해 목표 언어로 번역합니다.
- YouTube 원본 자막 트랙도 영어를 강제로 우선하지 않고 `serving` 상태를 우선합니다.
- 추천 언어는 영어·러시아어를 포함한 10개이며, 기본 선택 `ko`, `ja`, `es`는 유지합니다.
- 사용자가 현재 선택한 번역 언어 조합을 `내 기본 번역 언어`로 저장할 수 있습니다. 언어 코드만 브라우저 localStorage에 보관하며 비밀정보는 저장하지 않습니다.
