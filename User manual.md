...
## 10. 화면 레이아웃 보완
v1.7.0에서는 연결 관리의 큰 제목과 설명이 한글 어절 단위로 더 자연스럽게 줄바꿈되고, 작업공간 상단의 현재 채널 바와 본문 사이 간격이 확보됩니다. 데스크톱에서는 오른쪽 YouTube 업로드 영역과 현재 작업 요약이 스크롤 중 서로 겹치지 않도록 한 열로 함께 고정됩니다.
