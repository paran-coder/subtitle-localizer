...
const STATIC_STEPS: Record<number, StaticStep> = {
  ...
  3: {
    title: STEP_TITLES[2], eyebrow: "APP INFO", summary: "Google이 꼭 요구하는 기본 정보만 입력합니다.",
    href: "https://console.cloud.google.com/auth/overview", cta: "Google Auth Platform 열기 ↗",
    instructions: <><p>처음이면 <b>시작하기</b>를 누르세요. 앱 이름은 <code>{PROJECT_NAME}</code>, 사용자 유형은 <b>외부 · External</b>로 선택합니다.</p><p>지원 이메일과 개발자 연락처에는 본인이 사용하는 이메일을 선택·입력하고, 마지막 정책 동의 후 <b>만들기</b>까지 완료하세요.</p></>
  },
  4: {
    title: STEP_TITLES[3], eyebrow: "PUBLISHING", summary: "나중에 계정을 추가할 때 Cloud Console에 다시 갈 일을 줄입니다.",
    href: "https://console.cloud.google.com/auth/audience", cta: "Audience 열기 ↗",
    instructions: <><p><b>추천: In Production</b>으로 전환하세요. 그러면 정상적인 추가 계정 연결 때 Test user를 계속 추가하는 번거로움을 줄일 수 있습니다.</p><details className="v17-inline-details"><summary>Testing으로 계속 사용하려면</summary><p>실제로 로그인할 Google 계정을 Test users에 추가해야 합니다. 추가 계정이 생기면 이 화면에 다시 와야 할 수 있습니다. 검증 신청 자체는 이번 설정의 필수 조건으로 두지 않습니다.</p></details></>
  },
  5: {
...
export default function ConnectionsV17() {
  ...
  useEffect(() => {
    ...
    const messages: Record<string, string> = {
      connected: "YouTube 채널을 연결했습니다. 이제 같은 Cloud 설정으로 다른 채널도 계속 추가할 수 있습니다.",
      ...
      "client-not-configured": "먼저 Google Cloud 설정을 완료해 주세요.",
  ...
  async function saveGoogleConfig() {
    ...
    try {
      ...
      setRedirectUri(data.redirectUri || redirectUri); setGoogleClientSecret(""); setConnections([]); setActiveConnectionId(""); setYoutubeStatus("disconnected"); setWizardStep(8); setShowFinalWizardStep(true); setYoutubeMessage("Cloud 설정을 저장했습니다. 마지막으로 첫 YouTube 채널을 연결하세요.");
  ...
  async function forgetGoogle() {
    ...
    setYoutubeStatus("unconfigured"); setConnections([]); setActiveConnectionId(""); setGoogleClientId(""); setGoogleClientSecret(""); setRememberGoogle(false); setWizardStep(1); setConfirmed({}); setShowFinalWizardStep(false); setYoutubeMessage("Google Cloud 설정과 모든 YouTube 채널 연결을 지웠습니다.");
  }
  ...
  async function disconnectChannel(connectionId: string) {
    ...
    try {
      ...
      const next = data.connections ?? []; setConnections(next); setActiveConnectionId(data.activeConnectionId ?? ""); setYoutubeStatus(next.length ? "connected" : "disconnected"); setYoutubeMessage(next.length ? "선택한 채널 연결만 해제했습니다." : "YouTube 채널 연결을 모두 해제했습니다. Cloud 설정은 유지됩니다.");
  ...
  return <main className="v17-connections-shell">
    ...
    <section className="v17-connections-hero"><span>CONNECTIONS</span><h1>처음 한 번만 설정하고, 이후에는 채널만 추가하세요.</h1><p>Google Cloud는 사용자가 직접 소유합니다. 앱은 초보자가 판단할 일을 줄이고 꼭 필요한 클릭과 입력만 순서대로 안내합니다.</p></section>
    ...
    <section className="v17-card" id="youtube-connection">
      ...
      {youtubeStatus === "connected" && <div className="v17-channel-manager">
        <div className="v17-channel-manager-hero"><div><span>YOUTUBE CHANNELS · READY</span><strong>Google Cloud 설정은 끝났습니다.</strong><p>정상적인 채널 추가 때문에 Cloud Console로 돌아갈 필요가 없습니다.</p></div><a className="v17-primary" href="/api/youtube/oauth/start">+ 계정 또는 채널 추가</a></div>
        ...
        <details className="v17-cloud-details"><summary>Google Cloud 설정 · 완료됨</summary><p>OAuth Client를 삭제하거나 Client Secret을 교체한 경우처럼 기존 Cloud 설정 자체가 더 이상 유효하지 않을 때만 다시 설정하세요.</p><div className="v17-actions"><a className="v17-secondary" target="_blank" rel="noreferrer" href="https://console.cloud.google.com/auth/clients">Google Auth Platform 열기 ↗</a><button className="v17-text-danger" type="button" onClick={() => void forgetGoogle()}>Cloud 설정과 모든 채널 지우기</button></div></details>
      </div>}

      {youtubeStatus === "disconnected" && !showFinalWizardStep && <div className="v17-cloud-ready"><span>CLOUD SETUP · COMPLETE</span><strong>Cloud 설정은 이미 끝났습니다.</strong><p>다시 설정하지 마세요. 사용할 Google 계정 또는 YouTube 채널만 추가하면 됩니다.</p><a className="v17-primary" href="/api/youtube/oauth/start">+ 계정 또는 채널 연결</a><details className="v17-cloud-details"><summary>Cloud 설정을 바꿔야 하는 경우</summary><p>OAuth Client를 삭제했거나 Client Secret을 바꾼 경우에만 기존 설정을 지우고 다시 진행합니다.</p><button className="v17-text-danger" type="button" onClick={() => void forgetGoogle()}>기존 Cloud 설정 지우기</button></details></div>}

      {wizardVisible && <div className="v17-wizard">
        <Progress step={wizardStep} />
        <section className="v17-current-step">
          <div className="v17-current-head"><div><span>{wizardStep}/8 · {currentStatic?.eyebrow || (wizardStep === 7 ? "CREDENTIALS" : "FIRST CHANNEL")}</span><h3>{currentStatic?.title || STEP_TITLES[wizardStep - 1]}</h3><p>{currentStatic?.summary || (wizardStep === 7 ? "발급받은 두 값만 저장합니다." : "Google 계정을 승인하면 초기 설정이 끝납니다.")}</p></div><b>{wizardStep}</b></div>
          ...
          {wizardStep === 8 && <div className="v17-step-body v17-final-step"><div className="v17-final-checks"><span>프로젝트 ✓</span><span>YouTube API ✓</span><span>OAuth Client ✓</span><span>Client 저장 ✓</span></div><strong>이제 Google 계정 하나만 승인하면 끝입니다.</strong><p>앞으로 계정이나 채널을 추가할 때는 1~7단계를 다시 하지 않습니다.</p><a className="v17-primary wide" href="/api/youtube/oauth/start">Google로 첫 YouTube 채널 연결</a><small>Google 로그인에서는 실제 작업할 YouTube 채널이 있는 계정을 선택하세요.</small></div>}
