...
const STYLE_INSTRUCTIONS: Record<TranslationStyle, string> = {
  ...
  concise: "Make the subtitle concise and easy to read within a short on-screen duration. Remove verbal filler when it does not change meaning.",
...
function validateTranslationItems(
...
export async function translateChunk(input: TranslateChunkInput): Promise<TranslationItem[]> {
  ...
  const instructions = [
    ...
    "The contextBefore/contextAfter arrays are context only. Never output translations for those context items.",
    "Never merge, split, add, remove, renumber, or reorder cue IDs.",
    "Return exactly one translated text for every input cue ID.",
    "Preserve names, brands, URLs, technical identifiers, and meaningful punctuation unless localization requires a conventional form.",
    "Do not add explanations, translator notes, labels, markdown, or quotation marks around the subtitle text.",
