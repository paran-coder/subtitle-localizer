...
### 구현
- [ ] 현재 YouTube 작업 채널 바의 `v1.7.0` 삭제
- [ ] 공통 타이틀바의 `v1.7.0` 유지
- [ ] 공통 타이틀바 우측 `연결 관리` / `작업하기` CTA 삭제
- [ ] 페이지 이동은 `초기 설정 / 연결 관리 / 작업하기` 탭만 담당
- [ ] OpenAI/YouTube 상태와 파일 비저장 안내 유지
- [ ] 초기 설정 준비물 카드의 SRT 보조문장 데스크톱 한 줄 처리
- [ ] 모바일에서는 보조문장 자연 줄바꿈 허용
- [ ] 기존 채널 전환/채널 추가/연결/번역 로직 변경 없음

### 검증
- [ ] 자동 테스트 전체 통과
- [ ] TypeScript 통과
- [ ] Next.js Production build 통과
- [ ] Production `/`에서 작업 채널 바 버전 미노출 확인
- [ ] Production 주요 화면에서 타이틀바 페이지 이동 CTA 미노출 확인
- [ ] Production `/guide`에서 준비물 보조문장 데스크톱 한 줄 스타일 확인
