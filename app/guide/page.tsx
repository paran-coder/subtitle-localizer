...
const AUTHORIZED_DOMAIN = "subtitle-localizer.vercel.app";
      ...
      <div className="guide-content">
        ...
        <section id="youtube" className="guide-section">
          ...
          <div className="guide-steps">
            ...
            <Step number={3} title="Google Auth Platform의 앱 정보와 Branding을 채웁니다."><p>처음이면 `시작하기`를 누르고 앱 이름은 <code>Subtitle Localizer</code>, 사용자 유형은 <strong>External</strong>로 설정합니다. 지원 이메일과 개발자 연락처에는 본인이 사용하는 이메일을 지정합니다.</p><div className="guide-values"><ValueBox label="홈페이지" value={APP_URL} /><ValueBox label="개인정보처리방침" value={PRIVACY_URL} /><ValueBox label="서비스 약관" value={TERMS_URL} /></div><div className="guide-note"><strong>승인된 도메인도 꼭 추가하세요.</strong><span>Branding의 <strong>승인된 도메인</strong>에서 <strong>+ 도메인 추가</strong>를 누른 뒤 아래 값을 입력합니다. <code>https://</code>나 <code>/privacy</code>, <code>/terms</code> 같은 경로는 넣지 않습니다.</span></div><ValueBox label="승인된 도메인 · 복사용 값" value={AUTHORIZED_DOMAIN} /><div className="guide-note"><strong>이 단계가 빠지면</strong><span><code>누락된 도메인: subtitle-localizer.vercel.app</code> 경고가 표시될 수 있습니다.</span></div><a className="guide-action-link" href="https://console.cloud.google.com/auth/overview" target="_blank" rel="noreferrer">Google Auth Platform 열기 ↗</a></Step>
